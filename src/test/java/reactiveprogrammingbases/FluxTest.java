...
public class FluxTest {
    ...
    // zip will stop zip when the shortest flux element is rich (in case we have a flux with different element size)
    @Test
    public void zipOperator()  {
        Flux<String> titleFlux = Flux.just("G Blue", "Baki");
        Flux<String> studioFlux = Flux.just("Zero-G", "Entertainment");
        Flux<Integer> episodesFlux = Flux.just(12, 14);
        ...
        // animeFlux.subscribe(anime -> log.info(anime.toString()));  // uncomment to see what happen

        StepVerifier
                .create(animeFlux)
                .expectSubscription()
                .expectNext(new Anime("G Blue", "Zero-G", 12), new Anime("Baki", "Entertainment", 14))
                .verifyComplete();
    }

    // with only two flux or mono
    @Test
    public void zipWithOperator()  {
        ...
        StepVerifier
                .create(animeFlux)
                .expectSubscription()
                .expectNext(new Anime("G Blue", null, 12), new Anime("Baki", null, 14))
                .verifyComplete();
    }
    @AllArgsConstructor
    @NoArgsConstructor
    ...
    /*****************************************************************************
     * BlockHound is a library
     * it help to see if we are not doing something wrong that is blocking the thread
     * We are going to put it in MonoTest.java
     * ****************************************************************************/
